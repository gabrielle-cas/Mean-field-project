...
eqs="""
dv/dt = (g_iz*(v-E_iz)**2-u + J_E * s_E + J_I * s_I - I + eta) / Tv : 1 (unless refractory)
du/dt=(a*(b*v-u))/Tu:1
ds_E/dt = -s_E / Tsyn : 1
ds_I/dt = -s_I / Tsyn : 1  
a:1
...
S_12 = Synapses(G_inh, G_exc, on_pre='s_I_post += Ii') #'v_post -= 1.*mV')
S_12.connect('i!=j', p=prbC)

S_11 = Synapses(G_inh, G_inh, on_pre='s_I_post += Ii')
S_11.connect('i!=j',p=prbC)

S_21 = Synapses(G_exc, G_inh, on_pre='s_E_post += Ie')
S_21.connect('i!=j',p=prbC)

S_22 = Synapses(G_exc, G_exc, on_pre='s_E_post += Ie')
S_22.connect('i!=j', p=prbC)

S_ed_in = Synapses(P_ed, G_inh, on_pre='s_E_post += Ie')
S_ed_in.connect(p=.05)

S_ed_ex = Synapses(P_ed, G_exc, on_pre='s_E_post += Ie')
S_ed_ex.connect(p=.05)
